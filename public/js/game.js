...
export function startGameLoop(spaceship, road) {
  ...
  // (time - lastTime) 50Hz => time 34401.8
  // clock.getDelta() 50Hz => time 18230.3
  // clock.getDelta() 75Hz => time 35374.6
  // (time - lastTime) 75Hz => time 18191.1
  // clock.getDelta() 75Hz => time 18201.5

  function animate(time) {
    // Calculate delta time in seconds
    // let deltaTime2 = (time - lastTime) / 1000; // divide by 1000 to convert milliseconds to seconds
    let deltaTime = clock.getDelta();
    // console.log("deltaTime", deltaTime);
    // console.log("deltaTime2", deltaTime2);
    // lastTime = time;

    // Cap deltaTime to avoid large jumps (e.g. on tab switch)
    deltaTime = Math.min(deltaTime, 0.1); // Max out at 0.1 seconds (10 FPS)
    // const coef = deltaTime * 74.08;
    const coef = deltaTime * 100;
    gameSpeed += 0.0014 * coef;
    // gameSpeed += deltaTime / 10;
    if (gameSpeed > maxGameSpeed) {
      gameSpeed = maxGameSpeed;
    }
    // console.log("gameSpeed", gameSpeed);
    // console.log('gameSpeed before', gameSpeed);
    // gameSpeed = gameSpeed * coef;
    // // gameSpeed *= deltaTime * 80;
    ...
    if (!isGameOver) {
      ...
      if (controllerInput) {
        if (Math.abs(controllerInput.leftStick) > joystickDeadzone) {
          velocity +=
            (controllerInput.leftStick - Math.sign(controllerInput.leftStick) * joystickDeadzone) *
            acceleration *
            joystickSensitivity;
          targetRotation =
            -(controllerInput.leftStick - Math.sign(controllerInput.leftStick) * joystickDeadzone) *
            maxSpaceshipRotation *
            joystickSensitivity;
        }
      ...
      // Adjust game speed
      const adjustedGameSpeed = gameSpeed * coef;
      console.log('adjustedGameSpeed', adjustedGameSpeed);

      currentRoad.material.uniforms.offset.value += adjustedGameSpeed * 0.001;
      ...
      currentSpaceship.position.x += velocity * adjustedGameSpeed * 0.5;
      ...
      currentSpaceship.rotation.z += (targetRotation - currentSpaceship.rotation.z) * rotationSpeed * adjustedGameSpeed;
      ...
      score += Math.floor(adjustedGameSpeed * 10);
      updateScoreDisplay();

      spawnInterval = 1000 - adjustedGameSpeed * 360; // 646 = difficulty
      if (time - lastSpawnTime > spawnInterval) {
        createEnemy(adjustedGameSpeed);
        lastSpawnTime = time;
      }

      TWEEN.update(time);
      updateEnemies(adjustedGameSpeed);
      ...
      for (let i = 0; i < 10; i++) {
        if (i < enemies.length) {
          enemyPositions.push(enemies[i].position.x, enemies[i].position.y, enemies[i].position.z);
        } else {
      ...
      if (currentSpaceship) {
        ...
        currentSpaceship.traverse((child) => {
          if (child.isMesh && child.material.uniforms) {
            ...
            child.material.uniforms.gameSpeed.value = adjustedGameSpeed;
          }
        });
      }
...
function checkCollisions(spaceship) {
  ...
  const spaceshipShrinkFactor = 0.05;
...
function gameOver() {
  ...
  function checkForRestart() {
    ...
    if ((controllerInput && controllerInput.retry) || keys["Space"]) {
      ...
      // Only continue checking if the game is still over
...
function updateScoreDisplay() {
  ...
  if (score > bestScore && !scoreDisplayElement.classList.contains("new-best-score")) {
